...
def build(encoder, nc, momentum=0.9):
    ...
    enet = bottleneck(network, 64, upsample=True, reverse_module=index_stack.pop(), momentum=momentum)  # bottleneck 4.0
    enet = bottleneck(enet, 64, momentum=momentum)  # bottleneck 4.1
    enet = bottleneck(enet, 64, momentum=momentum)  # bottleneck 4.2
    enet = bottleneck(enet, 16, upsample=True, reverse_module=index_stack.pop(), momentum=momentum)  # bottleneck 5.0
